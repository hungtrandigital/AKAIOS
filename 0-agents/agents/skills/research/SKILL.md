...
1. **Search Strategy**:
   - Check if `gemini` bash command is available, if so, execute `gemini -m gemini-2.5-flash -p "...your search prompt..."` (timeout: 10 minutes).
...
- Prefer updating existing canonical files first (e.g., `1-ideas/1.1-market-research/summaries.md`, `2-product-foundation/product-backlog/backlog.md`).
